...
export const useBlockFeetypeStore = defineStore('main', {
    ...
    actions: {
        async getContents(dcb, feeTypeCd, page = 1) {
            const API_URL = "http://192.168.10.134:8080/block/fee";
            try {
                const response = await axios.get(API_URL, {
                    params: { dcb, feeTypeCd, page, pageSize:this.pageSize }
                });
        ...
        async deleteContents(dcb, inputtedKeyword, feeTypeCds) {
            const API_URL = "http://192.168.10.134:8080/block/fee";
            try {
                ...
                const remainingItems = this.total - feeTypeCds.length;
        ...
        async createContent(dcb, inputtedKeyword, feeTypeCd, feeTypeNm) {
            const API_URL = "http://192.168.10.134:8080/block/fee";
            try {
                await axios.post(API_URL, {
                    dcb, feeTypeCd, feeTypeNm, mbrId: '동욱스',
                });
