<template>
    <div class="content-wrapper">
        ...
        <v-row align="center" justify="end">
            <v-col cols="auto">
                <v-text-field class="min-width-input" v-model="inputtedKeyword" label="이벤트명"
            ...
            <div>
                <table>
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="(content, index) in contents" :key="content.eventName">
                            <td>{{ (currentPage - 1) * pageSize + index + 1 }}</td>
                            <td><input type="checkbox" v-model="selectedEvent" :value="content.eventName" /></td>
        ...
        <v-dialog v-model="isDialogVisible" max-width="600px">
            <v-card>
                <v-card-title class="text-h5">이벤트 등록</v-card-title> <br>
                <v-card-text>
                    <v-text-field label="이벤트명" v-model="form.event" /> <br>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
...
import { useEventStore } from '@/plugins/stores/event/event-gdcb'
...
const deleteContents = () => {
    ...
    if (isConfirmed) {
        store.deleteContents(selectedDcb.value, inputtedKeyword.value, selectedEvent.value).then(() => {
            selectedEvent.value = [];
        });
    }
};
...
const save = () => {
    if (validateForm()) {
        store.createContent(selectedDcb.value, inputtedKeyword.value, form.value.event);
        closeDialog();
    }
};
