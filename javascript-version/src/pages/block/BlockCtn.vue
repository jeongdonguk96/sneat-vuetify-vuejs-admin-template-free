<template>
    <div class="content-wrapper">
        ...
        <v-dialog v-model="isDialogVisible" max-width="600px">
            <v-card>
                <v-card-title class="text-h5">차단 CTN 등록</v-card-title> <br>
                <v-card-text>
...
const validateForm = () => {
    ...
    return !ctnError.value;
};


const save = () => {
    if (validateForm()) {
        store.createContent(selectedDcb.value, inputtedKeyword.value, form.value.ctn);
        closeDialog();
    }
};
</script>
